"""
JIRA AGENT — OpenAI Edition
Token-optimised Jira assistant for ADL project.
LLM: gpt-4o-mini | Auth: config.env
"""
...
def load_environment() -> dict[str, str]:
    """Load config.env / .env and validate required keys."""
    script_dir = Path(__file__).resolve().parent
    for env_path in [script_dir / ".env", script_dir / "config.env"]:
        if env_path.exists():
            load_dotenv(dotenv_path=env_path)
            logger.info("Loaded %s", env_path.name)
            break
    else:
        logger.error("No .env or config.env found in %s", script_dir)
        sys.exit(1)
    ...
    logger.info("Environment OK — project=%s  model=claude-haiku-4-5", DEFAULT_PROJECT)
...
def main() -> None:
    ...
    print("\n" + "=" * 60)
    print("  ADL Jira Agent  |  gpt-4o-mini  |  ASL Airlines")
    print("=" * 60)
