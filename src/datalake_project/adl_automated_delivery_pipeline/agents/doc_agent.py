...
Uses GPT-4o (single structured JSON call) to populate every section, then
assembles the Word document with python-docx applying proper heading levels,
bullet lists, and compact tables.

Output directory: D:\\Agents\\Project Documentation\\
...
# Allow both direct execution and import — check multiple locations, load all that exist
_ROOT = Path(__file__).resolve().parent.parent.parent
_AGENTS_DIR = Path(__file__).resolve().parent
for _candidate in [
    _ROOT / "config.env",
    _ROOT / ".env",
    _AGENTS_DIR / "config.env",
]:
...
def _add_key_value(doc: Document, label: str, value: str) -> None:
...
class DocumentationAgent:
    ...
    Uses GPT-4o in a single structured JSON call to fill in all document
    ...
    Usage::
        ...
        from adl_automated_delivery_pipeline.workflows.workflow import TicketRequirements
    ...
    def __init__(self) -> None:
        ...
        logger.info("DocumentationAgent initialised with model=claude-haiku-4-5-20251001")
    ...
    def _generate_content(
        ...
        """Call GPT-4o once to produce all document section content as JSON.
    ...
    def _save_document(self, doc: Document, ticket_id: str) -> Path:
