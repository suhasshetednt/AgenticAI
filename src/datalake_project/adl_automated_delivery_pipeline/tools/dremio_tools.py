...
def list_catalog_children(path: str) -> dict:
    ...
    try:
        ...
        if not raw_children:
            item_id = item.get("id")
            if item_id:
                detail = client.get(f"/catalog/{requests.utils.quote(item_id, safe='')}")
...
def create_dremio_folder(space: str, folder_path: str) -> dict:
    ...
    for seg in segments:
        built.append(seg)
        url_path = "/".join(requests.utils.quote(p, safe="") for p in built)
        try:
            ...
            if e.response.status_code != 404:
                return {
                    "status": "FAILED",
                    "error": f"Unexpected HTTP {e.response.status_code} checking {'.'.join(built)}: {e.response.text}",
                }
            ...
            for payload in [
                ...
                try:
                    ...
                    last_error = f"HTTP {ce.response.status_code}: {ce.response.text}"
            if last_error:
                return {
                    "status": "FAILED",
...
def update_virtual_dataset(vds_id: str, sql: str) -> dict:
    ...
    try:
        client = _new_client()
        encoded_id = requests.utils.quote(vds_id, safe="")
