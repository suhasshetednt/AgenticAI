...
def _execute_single(jira: JIRA, mutation: JiraMutation) -> dict:
    ...
    if op == "UPDATE_TICKET":
        issue = jira.issue(mutation.issue_key)
        issue.update(fields=p)
        return {"updated": mutation.issue_key}

    if op == "TRANSITION":
        transitions = jira.transitions(mutation.issue_key)
        ...
        jira.transition_issue(mutation.issue_key, matched["id"])
        return {"transitioned": mutation.issue_key, "to": target}

    if op == "ASSIGN":
        jira.assign_issue(mutation.issue_key, p.get("assignee_email"))
        return {"assigned": mutation.issue_key, "to": p.get("assignee_email")}

    if op == "ADD_COMMENT":
        comment = jira.add_comment(mutation.issue_key, p.get("comment_body", ""))
    ...
    if op == "ADD_TO_SPRINT":
        ...
        resp = requests.post(
            url,
            json={"issues": [mutation.issue_key]},
            auth=(settings.JIRA_USERNAME, settings.JIRA_API_TOKEN),
            headers={"Content-Type": "application/json"},
            timeout=30,
        )
