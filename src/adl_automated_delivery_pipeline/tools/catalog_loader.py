...
# Catalog JSON files live in settings.CATALOG_DIR (defaults to the project root).
# Override with CATALOG_DIR in config.env to point elsewhere.
_CATALOG_DIR = Path(settings.CATALOG_DIR)
_CATALOG_JSON = _CATALOG_DIR / "catalog_data.json"
_EXCEL_MASTER = _CATALOG_DIR / "excel_master.json"
...
# Public alias — `_load_raw` is cached internals; this is the supported entry point
# for other modules that need the unfiltered catalog tuple.
load_raw = _load_raw
