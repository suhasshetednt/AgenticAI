...
class Settings(BaseSettings):
    ...
    # ── Centralized Memory Platform (best-effort; pipeline never blocks on it) ──
    # Provision a key on the platform first: scripts/create_api_key.py --tenant adl
    MEMORY_ENABLED: bool = False           # off by default → zero behaviour change
    MEMORY_URL: str = "http://localhost:8000"
    MEMORY_API_KEY: str = ""
    MEMORY_TIMEOUT_S: float = 4.0          # hard cap — memory must not stall delivery
    MEMORY_AUTO_APPLY: bool = True         # apply engine-trusted SQL fixes; kill-switch = false
    MEMORY_TENANT: str = "adl"
    MEMORY_DOMAIN: str = "dremio"
